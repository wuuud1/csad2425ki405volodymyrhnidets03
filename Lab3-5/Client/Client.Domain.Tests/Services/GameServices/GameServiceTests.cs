...
using Client.Domain.Tests.Data;
using FakeItEasy;
...
public class GameServiceTests
{
    ...
    /// <summary>
    /// Tests the <see cref="GameService.SetGameState"/> method..
    /// Verifies that an <see cref="ArgumentNullException"/> is thrown for null game state.
    /// </summary>
    [TestMethod]
    [DataRow(null)]
    public void SetGameState_ShouldThrowArgumentNullException_WhenInvokeWithNewGameState(GameState state)
    {
        Assert.ThrowsException<ArgumentNullException>(() => _gameService.SetGameState(state));
    }
    ...
    /// </summary>
    [TestMethod]
    public void SetGameState_ShouldChangeCurrentGameState_WhenInvokeWithNewGameState()
    {
        GameState newState = new GameState();
        newState.Board = new bool?[,] { { true, true, true }, { false, false, null }, { null, null, null } };
        newState.Mode = GameMode.ManvsMan;
        newState.Status = GameStatus.WonPlayerX;

        _gameService.SetGameState(newState);

        Assert.AreEqual(_gameService.GetGameState(), newState);
    }
    ...
    [TestMethod]
    [DynamicData(nameof(GameServiceTestsDataProvider.XPlayerWinnerBoards), typeof(GameServiceTestsDataProvider))]
    public void IsWinner_ReturnSideX_WhenWinnerIsPlayerX(bool?[,] gameBoard)
    {
        bool? xSide = true;
        ...
        bool? isWinner = _gameService.IsWinner();

        Assert.AreEqual(xSide, isWinner);
    }

    [TestMethod]
    [DynamicData(nameof(GameServiceTestsDataProvider.OPlayerWinnerBoards), typeof(GameServiceTestsDataProvider))]
    public void IsWinner_ReturnSideO_WhenWinnerIsPlayerO(bool?[,] gameBoard)
    {
        bool? oSide = false;
        ...
        Assert.AreEqual(oSide, isWinner);
    }

    [TestMethod]
    [DynamicData(nameof(GameServiceTestsDataProvider.NoWinnerBoards), typeof(GameServiceTestsDataProvider))]
    public void IsWinner_ReturnEmptyCell_WhenWinnerNotExist(bool?[,] gameBoard)
    {
        bool? emptyCell = null;
        ...
        Assert.AreEqual(emptyCell, isWinner);
    }

    [TestMethod]
    public void AddReceivedEventHandler_ThrowArgumentNullException_WhendlerNull()
    {
        Assert.ThrowsException<ArgumentNullException>(() => _gameService.AddReceivedEventHandler(null));
    }

    [TestMethod]
    public void AddReceivedEventHandler_InvokeAddSerialDataReceivedEventHandlerInPortSettingWithHandler_WhenHandlerNotNull()
    {
        var handler = A.Fake<SerialDataReceivedEventHandler>();

        _gameService.AddReceivedEventHandler(handler);

        A.CallTo(() => _portSettings.AddSerialDataReceivedEventHandler(handler)).MustHaveHappenedOnceExactly();
    }
}
